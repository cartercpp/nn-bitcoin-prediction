...
#ifndef BITCOINNN_MATRIX_H
#define BITCOINNN_MATRIX_H
...
public:
        ...
        : m_data{data}, m_rows{data.size()}, m_columns{(data.size() > 0) ? data.begin()->size() : 0}
    {
        for (const auto& row : data)
            if (row.size() != m_columns)
                throw std::invalid_argument{"Each row in a vector must have the same # of columns"};
    }
    ...
    [[nodiscard]] math_vector<ValueType>& operator[](std::size_t row)
    {
        if (row >= m_rows)
    ...
    [[nodiscard]] const math_vector<ValueType>& operator[](std::size_t row) const
    {
        if (row >= m_rows)
            throw std::out_of_range{"Out of bounds"};
    ...
    {
        std::for_each(m_data.begin(), m_data.end(), [scalar](math_vector<ValueType>& rowRef){rowRef *= scalar;});
        return *this;
    }
...
auto operator*(const matrix<ValueType>& mat, const math_vector<ValueType>& vec)
{
    if (mat.columns() != vec.size())
        throw std::invalid_argument{
            "In order to do matrix-vector multiplication, the matrix must have as many columns as the vector"
            " has elements"
        };
